...
export function useCreateContributionPayment() {
  return useMutation({
    mutationFn: async (input: CreateContributionPaymentInput) => {
      const { data, error } = await supabase
        .from('contribution_payments')
        .insert({
      ...
      await logActivity({
        ...
        entityId: data.id,
        description: `Submitted payment of PKR ${input.amount} for ${input.month}`,
      })
    },
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: async ({ id, userId }: { id: string; userId: string }) => {
      ...
      await logActivity({
        ...
        entity: 'contribution_payment',
        entityId: id,
        description: 'Deleted contribution payment record',
      })
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: QUERY_KEYS.contributionPayments })
