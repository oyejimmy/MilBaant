  ...
  const columns: ColumnsType<CookRequest> = [
    {
      ...
      render: (_: unknown, r: CookRequest) => (
        <div>
          <Typography.Text
            strong
            style={{ color: "var(--text-strong)", display: "block" }}
          >
            {r.item}
          </Typography.Text>
          {r.quantity && (
          ...
          actions={
            <Space wrap>
              <Select
                value={filterStatus}
                onChange={(v) =>
                  setFilterStatus(v as CookRequestStatus | "all")
                }
                style={{ width: 140 }}
                options={[
          ...
          <SectionBlock>
            ...
            {filtered.length === 0 ? (
              ...
              <Space direction="vertical" size={8} style={{ width: "100%" }}>
                {filtered.map((r) => {
                  const canDelete =
                    (r.requested_by === userId || isAdmin) && !isCook;
                  return (
                    <MobileCard key={r.id}>
                      <MobileRow>
                        <Typography.Text
      ...
      {replyTarget && (
        <CookReplyModal
          ...
          onSubmit={(status, comment) =>
            void handleReply(replyTarget, status, comment)
          }
        />
      )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography.Title
          level={5}
          style={{ margin: "4px 0 0", color: "var(--text-strong)" }}
        >
          {request.item}
          {request.quantity && (
            <Typography.Text
