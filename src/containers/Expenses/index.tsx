            ...
            {userPayments.map((p) =>
              p.created_by === userId || isAdmin ? (
                <Popconfirm
                  ...
                  onConfirm={() => void handleDeletePayment(p.id)}
                >
                ...
                {weekendExpenses.map((exp) => (
                  <MobileCard key={exp.id}>
                    ...
                    <MobileRow>
                      ...
                      >
                        Share/person:{" "}
                        {formatCurrency(calculateWeekendExpenseShare(exp))}
                      </Typography.Text>
                      {isAdmin && (
                        <Popconfirm
                          title="Delete?"
                          onConfirm={() => void handleDeleteExpense(exp.id)}
                        >
                          <Button
                            danger
                            icon={<DeleteOutlined />}
            ...
            {isMobile ? (
              <Space direction="vertical" size={8} style={{ width: "100%" }}>
                {userSummary.map((row) => {
                  ...
                  const isOverdue =
                    !isPaid &&
                    dayjs().isAfter(dayjs(selectedMonth).endOf("month"));
                  return (
      ...
      >
        ...
        >
          ...
          <div
            style={{
              background:
                "linear-gradient(135deg, var(--primary) 0%, var(--blue-700) 100%)",
          ...
          >
            {[
              ...
              >
                ...
                <div
                  style={{ fontSize: 15, fontWeight: 700, color: pill.color }}
                >
                  {pill.value}
                </div>
              </div>
          ...
          <div style={{ padding: "0 0 20px" }}>
            ...
            >
              ...
              <tfoot>
                ...
                >
                  <td
                    style={{ ...pTd, fontWeight: 700, color: "var(--primary)" }}
                    colSpan={2}
                  >
                    Total
                  </td>
