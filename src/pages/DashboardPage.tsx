...
import {
  ArrowRightOutlined, BankOutlined, CalendarOutlined,
  CheckCircleOutlined, CreditCardOutlined,
  EditOutlined, HomeOutlined, PieChartOutlined,
  RiseOutlined, TeamOutlined, UserOutlined,
  WalletOutlined, FireOutlined, FundOutlined, HistoryOutlined,
  CopyOutlined,
...
import {
  useMemberCountSetting, useContributeInfo, usePrevMonthRemainder,
...
import {
  buildMonthlyUserSummary, calculateFixedTotal,
  calculatePerMemberShare, splitExpensesByType,
...
const PremiumChip = styled.div`
  ...
  position: relative; overflow: hidden; margin: 12px 0 10px;
`
...
const FlatFundFill = styled.div<{ $pct: number; $over: boolean }>`
  ...
  background: ${p => p.$over ? '#ff4d4f' : 'linear-gradient(90deg, #52c41a, #73d13d)'};
  border-radius: 6px;
  transition: width 0.4s ease;
`
...
export function DashboardPage() {
  ...
  const userSummary = buildMonthlyUserSummary(flatmates, perMemberShare, weekendExpenses)
  ...
  const paidCount = flatmates.filter(m => paymentExistsByUser.get(m.id) === true).length

  // Cook requests
  const pendingCookRequests = cookRequests.filter(r => r.status === 'pending').length
  ...
  async function handleSavePrevRemainder() {
    try {
      ...
      message.error(err instanceof Error ? err.message : 'Unable to update remainder.')
    }
  }
  ...
  return (
    <PageStack>
      ...
      <QueryState isLoading={isLoading} error={error}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: contributeInfo ? (isMobile ? '1fr' : '1fr 0.65fr') : '1fr', gap: 12, alignItems: 'stretch' }}>
          <DinnerCard>
            {/* top: badge */}
            <DinnerBadge><FireOutlined style={{ fontSize: 10 }} /> Tonight's Dinner</DinnerBadge>
        ...
        <Card>
          ...
          <FlatFundGrid>
            <FlatFundCell>
              <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                <BankOutlined /> Allocated
              </div>
              <div style={{ fontSize: isMobile ? 16 : 20, fontWeight: 800, color: 'var(--text-strong)' }}>
            ...
            <FlatFundCell>
              ...
              <div style={{ fontSize: isMobile ? 16 : 20, fontWeight: 800, color: '#ff7a45' }}>
            ...
            <FlatFundCell>
              ...
              <div style={{ fontSize: isMobile ? 16 : 20, fontWeight: 800, color: flatFundBalance >= 0 ? '#52c41a' : '#ff4d4f' }}>
                {flatFundBalance >= 0 ? '+' : ''}{formatCurrency(flatFundBalance)}
              </div>
            </FlatFundCell>
          </FlatFundGrid>
          ...
          {isMobile ? (
            ...
            <div>
              {flatmates.map(member => {
                ...
                const owed = summary ? Math.max(0, summary.totalOwed - paid) : 0
                  ...
                  <BalanceListItem key={member.id} $isMe={isMe}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-strong)', display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        {isMe && <span style={{ fontSize: 10, color: '#1677ff', fontWeight: 700 }}>you</span>}
                      </div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      {isPaid ? (
                        <span style={{ fontSize: 12, fontWeight: 700, color: '#52c41a' }}>✓ Paid</span>
                      ) : (
            ...
            <BalanceTable>
              ...
              <tbody>
                {flatmates.map(member => {
                  ...
                  const initials = member.full_name.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()
                  return (
                    <tr key={member.id} style={{ background: isMe ? 'rgba(22,119,255,0.04)' : undefined }}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            {isMe && <span style={{ fontSize: 10, color: '#1677ff', fontWeight: 700, marginLeft: 6 }}>you</span>}
                          </div>
                        </div>
                      </td>
                      <td style={{ fontWeight: 600 }}>{formatCurrency(summary?.totalOwed ?? 0)}</td>
                      <td style={{ color: '#52c41a', fontWeight: 600 }}>{formatCurrency(paid)}</td>
                      <td style={{ fontWeight: 700, color: isPaid ? '#52c41a' : '#ff4d4f' }}>
                      ...
                      <td>
                        <span style={{
                          ...
                          fontSize: 11,
                          fontWeight: 700,
                          background: isPaid ? 'rgba(82,196,26,0.1)' : 'rgba(255,77,79,0.08)',
      ...
      >
        ...
        <InputNumber
          ...
          onChange={(value) => setRemainderInput(typeof value === 'number' ? value : 0)}
          min={0}
          precision={2}
          style={{ width: '100%' }}
