...
const UploadIconWrap = styled.div<{ $hasFile: boolean }>`
  ...
  .anticon {
    font-size: 15px;
    color: ${({ $hasFile }) =>
      $hasFile ? "var(--success)" : "var(--primary)"};
  }
`;
...
const ParticipantChip = styled.div<{ $checked: boolean }>`
  ...
  border-radius: 10px;
  border: 1.5px solid
    ${({ $checked }) => ($checked ? "var(--primary)" : "var(--border-light)")};
  background: ${({ $checked }) =>
    $checked ? "var(--primary-soft)" : "var(--bg-elevated)"};
  cursor: pointer;
...
const CheckDot = styled.div<{ $checked: boolean }>`
  ...
  border-radius: 5px;
  border: 1.5px solid
    ${({ $checked }) => ($checked ? "var(--primary)" : "var(--border-default)")};
  background: ${({ $checked }) =>
    ...
    if (open) {
      if (editingExpense) {
        form.setFieldsValue({
          ...
          lastDate: editingExpense.last_date
            ? dayjs(editingExpense.last_date)
            : undefined,
          description: editingExpense.description ?? "",
          participantIds: editingExpense.expense_participants.map(
            (p) => p.user_id,
          ),
        });
      } else {
        form.setFieldsValue({
          ...
          lastDate: undefined,
          description: "",
          participantIds: flatmates.map((p) => p.id),
        });
      }
    }
  }, [form, open, editingExpense, lockedCategory, flatmates]);

  useEffect(() => {
    if (!isWeekendMeal) {
      form.setFieldValue(
        "participantIds",
        flatmates.map((p) => p.id),
      );
    }
  }, [form, isWeekendMeal, flatmates]);
      ...
      <FormBody>
        ...
        {category && (
          <TypeBadge $weekend={isWeekendMeal}>
            ...
            {isWeekendMeal
              ? "Weekend Meal — split among selected participants"
              : `Monthly Expense — ${categoryLabel}`}
          </TypeBadge>
        )}
          ...
          <Row>
            ...
            >
              <Select
                options={
                  lockedCategory
                    ? EXPENSE_CATEGORY_OPTIONS.filter(
                        (o) => o.value === lockedCategory,
                      )
          ...
          {isWeekendMeal && (
            <>
              ...
              <SectionLabel>
                ...
                >
                  {participantIds.length} / {flatmates.length} selected
                </Typography.Text>
              </SectionLabel>
