...
const COOK_NAV_ITEMS: CookNavItem[] = [
  { key: '/cook-portal/dashboard',        label: 'Dashboard',      icon: <DashboardOutlined /> },
...
const TopHeader = styled(Header)<{ $scrolled: boolean }>`
  ...
  background: ${({ $scrolled }) =>
    $scrolled ? 'rgba(var(--navbar-bg-rgb), 0.55)' : 'var(--navbar-bg)'}  ;
      ...
      onClick: () =>
        void (async () => {
          try {
            ...
            void message.error(e instanceof Error ? e.message : 'Unable to sign out.')
          }
        })(),
    },
  ]
        ...
        >
          ...
          <NavWrap>
            {profileLoading ? (
              <SkeletonWrap>
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <Skeleton.Button key={i} active block style={{ borderRadius: 8, height: 40 }} />
                ))}
              </SkeletonWrap>
            ) : (
              desktopMenu
            ...
            <Dropdown menu={{ items: profileMenuItems }} trigger={['click']} placement="topRight">
              <ProfileRow $collapsed={collapsed}>
                ...
                <ProfileMeta $visible={!collapsed}>
                  <Typography.Text
                    strong
                    style={{ color: 'var(--text-strong)', fontSize: '0.8rem', display: 'block' }}
                  >
                    {profile?.full_name ?? 'Cook'}
                  </Typography.Text>
                  <Typography.Text style={{ color: '#f97316', fontSize: '0.68rem', fontWeight: 600 }}>
                    Cook
                  </Typography.Text>
                </ProfileMeta>
              </ProfileRow>
        ...
        >
          <DrawerBrand>
            ...
            <div>
              <Typography.Text
                strong
                style={{ color: 'var(--text-strong)', fontSize: '0.95rem', display: 'block' }}
              >
                {APP_NAME}
              </Typography.Text>
              <Typography.Text style={{ color: '#f97316', fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                Cook Portal
              </Typography.Text>
            </div>
          </DrawerBrand>
        ...
        <TopHeader $scrolled={scrolled}>
          <NavLeft>
            ...
            <PageTitleWrap>
              ...
              {isDesktop && <CookBadge><CoffeeOutlined /> Cook</CookBadge>}
            </PageTitleWrap>
          </NavLeft>
              ...
              <ProfileBtn type="button" aria-label="Profile menu">
                ...
                {isDesktop && (
                  ...
                  >
                    <Typography.Text
                      strong
                      style={{ color: 'var(--text-strong)', fontSize: '0.78rem' }}
                    >
                      {profile?.full_name ?? 'Cook'}
                    </Typography.Text>
                    <Typography.Text style={{ color: '#f97316', fontSize: '0.68rem', fontWeight: 600 }}>
