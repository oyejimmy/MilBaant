...
import { shimmer, stagger, fadeUp } from "@/components/AuthShared";
...
const VARIANTS: Record<AuthVariant, VariantConfig> = {
  login: {
    accent: "#1c8ee5",
    badgeGradient:
      "linear-gradient(145deg, #1465a3 0%, #1c8ee5 55%, #2fa8f5 100%)",
    quote:
      '"Splitting bills fairly since forever.\nStill cheaper than living alone."',
  ...
  register: {
    accent: "#22c55e",
    badgeGradient:
      "linear-gradient(145deg, #15803d 0%, #22c55e 55%, #4ade80 100%)",
  ...
  forgot: {
    accent: "#f59e0b",
    badgeGradient:
      "linear-gradient(145deg, #b45309 0%, #f59e0b 55%, #fcd34d 100%)",
  ...
  reset: {
    accent: "#8b5cf6",
    badgeGradient:
      "linear-gradient(145deg, #6d28d9 0%, #8b5cf6 55%, #c4b5fd 100%)",
  ...
  pending: {
    accent: "#f97316",
    badgeGradient:
      "linear-gradient(145deg, #c2410c 0%, #f97316 55%, #fdba74 100%)",
    quote:
...
const Orb = styled.div<{
  ...
  animation: ${(p) =>
    p.$drift === "a"
      ? css`
          ${driftA} ${p.$duration}s ease-in-out infinite
        `
      : p.$drift === "b"
        ? css`
            ${driftB} ${p.$duration}s ease-in-out infinite
          `
        : css`
            ${driftC} ${p.$duration}s ease-in-out infinite
          `};
`;
...
const BrandName = styled.div`
  ...
  margin-bottom: 10px;
  opacity: 0;
  animation: ${fadeUp} 0.5s ease 200ms forwards;
`;
...
const BrandTitle = styled.h1`
  ...
  opacity: 0;
  animation: ${fadeUp} 0.5s ease 300ms forwards;
...
const BrandSub = styled.p`
  ...
  max-width: 270px;
  opacity: 0;
  animation: ${fadeUp} 0.5s ease 380ms forwards;
...
const Divider = styled.div`
  ...
  margin: 24px 0;
  opacity: 0;
  animation: ${fadeIn} 0.4s ease 440ms forwards;
...
const FeatureItem = styled.li<{ $delay: number }>`
  ...
  font-weight: 500;
  opacity: 0;
  animation: ${fadeUp} 0.4s cubic-bezier(0.22, 1, 0.36, 1) ${(p) => p.$delay}ms
    forwards;
...
const BrandFooter = styled.div`
  ...
  gap: 3px;
  opacity: 0;
  animation: ${fadeIn} 0.5s ease 900ms forwards;
...
const ThemeBtn = styled.div`
  ...
  z-index: 10;
  opacity: 0;
  animation: ${fadeIn} 0.4s ease 500ms forwards;
`;
...
const CardLogoTagline = styled.span`
  font-size: 11px;
  color: var(--text-muted);
  font-weight: 500;
`;
...
const FEATURES = [
  ...
  { icon: "👨‍🍳", label: "Flat Ka Khata  & advance management" },
    ...
    <Page>
      ...
      <BrandPanel>
        ...
        <BrandTop>
          ...
          <BrandSub>
            Expenses, meals, cook ledger, contributions — all in one place.
          </BrandSub>
        ...
        <BrandFooter>
          ...
          <FooterLove>
            Made with <span className="heart">❤️</span> by Jimmy
          </FooterLove>
        </BrandFooter>
      </BrandPanel>
